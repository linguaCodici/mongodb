class Entrant
  ...
  def update_total(result)
      Rails.logger.debug {"updating secs #{result} with results #{results}"}
      #Rails.logger.debug {"updating secs #{results.pluck(:secs)} of class #{results.pluck(:secs).class}"}
      # self.secs = results.pluck(:secs).sum # DEBUG: nil class error

      arr = results.map {|m| m.secs}
      self.secs = arr.to_a.reject(&:nil?).sum
  end
  ...
  RESULTS.keys.each do |name|
      ...
      # expose seter/getter for each property of each result
